...
def test_clean_run_investigates_and_writes_report(orders, tmp_path):
    ...
    r = run(orders, out_dir=str(tmp_path / "reports"), ask=NO_LLM, root=str(tmp_path / "ws"), memory_path=mem)
    assert r["outcome"] == "INVESTIGATED"
    assert len(r["memory"]["recorded"]) == 1 and r["memory"]["prior"] == []
    again = run(orders, out_dir=str(tmp_path / "reports"), ask=NO_LLM, root=str(tmp_path / "ws"), memory_path=mem)
